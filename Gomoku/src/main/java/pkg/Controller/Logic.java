...
public class Logic {
	 ...
	 * The winner fields.
	 */
	private String winFields[];

	/**
	 * Creates the {@code Game} object.
	 * 
	 * @param p1
	 *            the player who has X.
	 * @param p2
	 *            the player who O;
	 */
	public Logic(Player p1, Player p2) {
		...
		winFields= new String[5];
	}

	/**
	 * Getting the {@code currentPlayer}. With this, we can alternate who the
	 * current player is.
	 * 
	...
	/**
	 * Getting the {@code otherPlayer}. With this, we can alternate who the
	 * other player is.
	 * 
	...
	/**
	 * To check if there is a tie. Also check if the game was won, because if it
	 * was, then we dont check for tie, and return false.
	 * 
	...
	/**
	 * Returns the winner fields in a string array with the following format.
	 * ["x;y"],["x+1;y+1"]...
	 * 
	 * @param x col. num.
	 * @param y row num.
	 ...
	 * @return ["x;y"],["x+1;y+1"]..
	 */
	public String[] winnerFields(int x, int y, Direction dir) {
		String wF[] = new String[5];
		int idx = 0;
		...
		for (int y = 0; y < board.BOARD_SIZE; y++) {
			for (int x = 0; x < board.BOARD_SIZE; x++) {
				for (Direction dir : Direction.values()) {
					...
					if (scoreForX >= 5 ) {
						winFields=winnerFields(x,y,dir);
						breakAll = true;
						break;
					}
					if ( scoreForO >= 5) {
	 ...
	 *            X or O
	 * @param shouldbreak
	 *            if true, than we can stop iterating in
	 *            {@link #checkInDirection(int x, int y, Direction dir, char XorO)}
	...
	/**
	 * Gets x,y coordinates and from that position it loops through the
	 * {@code board} in the given {@code Direction}.
	 * 
	 * @param x
	 *            column number
	 * @param y
	 *            row number
	 * @param dir
	 *            The {@code Direction} in which it will search for X or O
	 * @param XorO
	 *            the character that we search in the given {@code Direction}
		...
		for (int i = 0; i < 5; i++) {
			...
			case Top:
				if (isInBoundaries(x, y)) {
					score += CheckIfCharIsInPlace(x, y, XorO, shouldbreak);
					y--;
				} else {
					shouldbreak = true;
				}
	...
	/**
	 * Getter for winner fields.
	 * @return ["x;y"]...
	 */
	public String[] getWinFields() {
		return winFields;
	}

	/**
	 * Setter for winner fields.
	 * @param winFields ["x;y"],["x+1;y+1"]...
	 */
	public void setWinFields(String[] winFields) {
